      ...
      <div className="mx-auto w-[min(1160px,calc(100%-32px))]">
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {projects.map((project, index) => (
            <article key={project.title} className="group overflow-hidden border   transition duration-300 hover:-translate-y-1 hover:border-emerald-400/40 hover:shadow-2xl border-white/10 bg-white">
              ...
              <div className="p-5">
                <div className="flex items-start justify-between gap-4">
                  <h3 className="text-lg font-black tracking-[-.03em] text-black">{project.title}</h3>
                  <span className="text-[9px] font-bold text-black">0{index + 1}</span>
                </div>
                <p className="mt-2 text-[12px] leading-6 text-zinc-600">{project.description}</p>

                <div className="mt-4 flex flex-wrap gap-1.5">
                  {project.tags.map(tag => (
                    <span key={tag} className="border px-2 py-1 text-[9px] font-semibold  border-white/10 text-zinc-600">{tag}</span>
                  ))}
                </div>

                <div className="mt-5 flex items-center justify-between border-t  pt-4 border-white/5">
                  <a href={project.github} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-[10px] font-bold hover:text-emerald-500 text-zinc-600">
